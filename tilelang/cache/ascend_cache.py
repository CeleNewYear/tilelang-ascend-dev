...
# 需要，用于构建kernel参数列表 dtype, shape
# # "param_info": self.param_info,


# "out_idx": self.out_idx,
# "symbolic": self.symbolic,
# "gridfunc": self.gridfunc,  # 需要，用于计算launch grid
# "mix_mode": self.mix_mode,
# "launch_packedMetadata": self.launch_packedMetadata,
# "utils_name": self.utils_name,
# "tensor_kinds": self.tensor_kinds,

# 根据cache path获得
# "so_launcher_path": self.so_launcher_path,  # 不需要，从换从文件获取即可
...
class KernelCache:
    ...
    execution_backend: Literal["torch"] = "torch"

    device_kernel_path = "device_kernel.cu"
    host_kernel_path = "host_kernel.cu"
    kernel_lib_path = "kernel_lib.so"

    kernel_src_tvm_path = "kernel.json"
    kernel_src_mlir_path = "kernel.mlir"
    kernel_object_path = "kernel.o"
    kernel_launcher_path = "kernel_launcher.so"
    ...
    def _get_compile_args() -> dict:
        ...
        return {
            "options": ["-x", "objective-c++", "-g", "-std=gnu++17"]
    ...
    def _generate_key(
        ...
        execution_backend: Literal[
            "tvm_ffi", "cython", "nvrtc", "torch", "cutedsl"
        ] = "tvm_ffi",
        args=None,
    ...
    def cached(
        ...
        *args,
        ...
        if verbose:
            self.logger.info(
                f"Generated cache key: {key} for kernel {get_prim_func_name(func, '<unknown>')}"
            )
        with self._lock:
            ...
            if verbose:
                self.logger.debug(
                    f"Checking disk cache for kernel {get_prim_func_name(func, '<unknown>')}"
                )
            ...
            if kernel is not None:
                if verbose:
                    self.logger.debug(
                        f"Found kernel in disk cache for {get_prim_func_name(func, '<unknown>')}"
                    )
        ...
        if verbose:
            self.logger.debug(
                f"No cached kernel for {get_prim_func_name(func, '<unknown>')}"
            )
    ...
    def _safe_write_executable(cls, executable: Executable, path: str):
        temp_path = os.path.join(
            env.TILELANG_TMP_DIR, f"{os.getpid()}_{uuid.uuid4()}.so"
        )
    ...
    def _save_kernel_source_code_to_disk(
        self, kernel: JITKernel, cache_path: str, verbose: bool = False
    ):
        device_kernel_path = os.path.join(cache_path, self.device_kernel_path)
        if verbose:
            self.logger.debug(
                f"Saving kernel source code to file: {device_kernel_path}"
            )
        if kernel.kernel_source is not None:
            KernelCache._safe_write_file(
                device_kernel_path, "w", lambda file: file.write(kernel.kernel_source)
            )
    ...
    def _save_wrapper_kernel_code_to_disk(
        ...
        if verbose:
            self.logger.debug(
                f"Saving wrapped kernel source code to file: {host_kernel_path}"
            )
        KernelCache._safe_write_file(
            host_kernel_path,
            "w",
    ...
    def _load_kernel_source(
        ...
        try:
            if verbose:
                self.logger.debug(
                    f"Loading kernel source code from file: {device_kernel_path}"
                )
    ...
    def _build_kernel(
        ...
        if missing_components:
            self.logger.warning(
                "Cannot build kernel from cache: missing required component(s): %s",
